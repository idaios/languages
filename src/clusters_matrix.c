...
pclusters_matrix clusters_create_matrix(int *source, size_t size, int verbose_flag, char** fastaNames) {
    ...
    for (size_t i = 0; i < size; ++i) {
      ...
      //      if (!clusters_update_name(new_instance, i, fastaNames[j-1], i)) {
	if (!clusters_update_name(new_instance, i, fastaNames[j-1], i)) {
	// In case of error
	clusters_free_distances(new_instance);
	return NULL;
